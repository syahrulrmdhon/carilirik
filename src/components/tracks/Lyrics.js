...
class Lyrics extends Component {
    ...
    componentDidMount() {
        axios
        .get(`https://cors-anywhere.herokuapp.com/http://api.musixmatch.com/ws/1.1/track.lyrics.get?track_id=${this.props.match.params.id}&apikey=${process.env.REACT_APP_MM_KEY}`)
        .then(res => {
    ...
    render() {
        ...
        if ( track === undefined || lyrics === undefined || Object.keys(track).length === 0 || Object.keys(track).length === 0 ){
            ...
            return (
                <React.Fragment>
                    <Link to='/' className="btn btn-dark btn-sm mb-4">Kembali</Link>
                    <h5 className="card">
                    ...
                    <div className='card-body'>
                        {lyrics.lyrics_body.split('\n').map((item, i) => {
                            return <p className="card-text" key={i}>{item}</p>;
                        })}
                    </div>
